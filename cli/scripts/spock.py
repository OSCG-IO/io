...
def create_replication_set(set_name, db, pg, replicate_insert=True, replicate_update=True, 
                           replicate_delete=True, replicate_truncate=True):
  pass


def create_subscription(subscription_name, provider_dsn, db, pg, replication_sets=None,
                        synchronize_structure=False, synchronize_data=False, 
                        forward_origins='{}', apply_delay=0):
  pass


def alter_subscription_add_replication_set(subscription_name, replication_set, db, pg):
  pass


def wait_for_subscription_sync_complete(subscription_nam, db, pg):
  pass
